...
public class WechatDecryptDataUtil {
    ...
    private static byte[] initIvs(byte[] keyBytes) {
        // 如果密钥不足16位，那么就补足.  这个if 中的内容很重要
        int base = 16;
        if (keyBytes.length % base != 0) {
            int groups = keyBytes.length / base + (keyBytes.length % base != 0 ? 1 : 0);
     ...
     * @return 解密后的字节数组
     */
    private static byte[] decryptOfDiyIV(byte[] encryptedData, byte[] keyBytes, String ivs) {
        byte[] encryptedText = null;
        init(keyBytes);
        try {
            byte[] iv = initIvs(ivs.getBytes("UTF-8"));
